...
int main(int argc, char *argv[])
{
	...
	char *inputFileName = "test.bmp", *outputFileName = NULL;
	...
	if (convertBack) {
		...
		if (fpInputFile == NULL) {
			printf
			    ("Can not open the input texture tile file %s\n",
			     inputFileName);
		}
		...
		do {
			rdPointer +=
			    fread(tileMapBuffer + rdPointer,
				  sizeof(uint8_t), length - rdPointer,
				  fpInputFile);
		} while (rdPointer != length);

		fclose(fpInputFile);

		ConvertTextureTileToBitmap(tileMapBuffer, width, height,
					   &outputBitmap->image);
		free(tileMapBuffer);

		BitmapWriteError errWrite =
		    WriteBitmap(outputBitmap, outputFileName);
		...
		BitmapReadError errRead =
		    ReadBitmap(inputFileName, &inputBitmap);
		...
		uint32_t length =
		    mat->width * mat->height * sizeof(uint32_t);
		uint8_t *tileMapBuffer = (uint8_t *) malloc(length);
...
{
	printf
	    ("This program can convert the bitmap image to store it as continuous 16x16 texture tiles.\n");
	...
	printf
	    ("\t               For convert to texture tiles file, this must be a Bitmap.\n");
	printf
	    ("\t               For convert back, this must be a Texture Tiles File.\n");
	printf("\t<output_file>: Output file.\n");
	printf("\n");
	printf("NOTE: The format of output texture tiles file: \n");
	printf
	    ("\t    There are 2 of 32-bit binary fields at the head of file, \n");
	printf
	    ("\t    indicated the width and height of the image (in pixel).\n");
	printf
	    ("\t    The 32-bit binary fields are stored as little-endian.\n");
	printf("\t    And the RGBA pixels are follow the head.\n");
	printf
	    ("\t    The fields of each pixel are arranged in order of R, G, B, A.\n");
}
